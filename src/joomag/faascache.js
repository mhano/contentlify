...
class FaaSCache {
    ...
    removeOldCacheEntries() {
        ...
        if (this.lastCacheCleanup < (Date.now() - this.minCleanupIntervalMs)) {
            ...
            console.log({ ts: (new Date()).toISOString(), "event": "cacheCleanup", traceProcId: this.traceProcId, objects: deletedObjectCount, pointers: deleteCount, duration: (Date.now() - now) });
        }
    }
}
